...
/* With algo -	RSA (Public Key/Private Key pair) */
	...
	if(checkAccount($acc,$pwd,$conn)){
		packageToken($acc);
		setcookie("failToLogin",null,time()-360000,"/","127.0.0.1:8080",0,0);
		echo $_COOKIE['token'];
		header("Location:http://127.0.0.1:8080/bootstrap-WSSE1/index.html");
	}else{
		$failToLogin = $_COOKIE["failToLogin"];
		$failToLogin += 1;
		setcookie("failToLogin",$failToLogin,time()+(300),"/","127.0.0.1:8080",0,0);
		header("Location:http://127.0.0.1:8080/bootstrap-WSSE1/login.html");
	...
	function packageToken($acc){
		...
		setcookie("token",$token,time()+(3600),"/","127.0.0.1:8080",0,0);
		echo $token;
	}
